...
const formItemList1 = [
  ...
  },
  {
    ...
    list: [
      { key: 0, value: "未选" },
      ...(userStore.doctorObj?.major_doctor || []),
    ],
  },
  // {
...
const isEdit = () => {
  return (
    userStore.isShowOrderType === "edit" ||
    userStore.isShowOrderType === "readd"
  );
};
  ...
  <div v-if="isShow" class="order-main">
    <div class="header">
      ...
      <div class="order-main-right">
        <el-button @click="userStore.updateIsShowOrder(false)">
          取消
        </el-button>
      ...
      >
        <div class="lr left">
          ...
          <div class="lr-header-detail">
            <div class="lr-header-detail-item">
              客户:{{ ruleForm?.patient_name }}
            </div>
            <div class="lr-header-detail-item">手机:{{ ruleForm?.mobile }}</div>
          </div>
          <div class="lr-header">预约信息</div>
          <com-form-item-list
            :form-item-list="formItemList1"
            :rule-form="ruleForm"
          ></com-form-item-list>
        </div>
        <div class="lr right">
          <div :style="{ marginBottom: '5px' }">
            ...
            <el-input
              v-model="inputSearch"
              placeholder=""
              @change="onChageSearch"
            />
          </div>
          <com-form-item-list
            :form-item-list="onFilterProject()"
            :rule-form="ruleForm"
          ></com-form-item-list>
        </div>
      </el-form>
