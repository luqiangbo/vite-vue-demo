    ...
    >
      <el-tab-pane label="预约看板" name="1">
        ...
        <div class="regain-main">
          <el-button
            v-if="userStore.timeType === 'date'"
            @click="onClickRegainDate"
          >
            回到今天
          </el-button>
          <el-button
            v-if="userStore.timeType === 'week'"
            @click="onClickRegainWeek"
          >
            回到本周
          </el-button>
        </div>
          ...
          <el-collapse v-model="orderCollapse">
            <el-collapse-item title="预约状态" name="1">
              <div class="check-list">
                <el-checkbox
                  v-model="userStore.filterStatusAll"
                  @change="onCheckAllStatus"
                  >全选</el-checkbox
                >
                <el-checkbox-group
                  v-model="userStore.filterStatusList"
                  @change="onCheckListStatus"
                >
                  <el-checkbox
                    v-for="u in userStore.orderStatusList"
                    :key="u.key"
                    :label="u.key"
                    >{{ u.value }}</el-checkbox
                  >
            ...
            <el-collapse-item title="医生筛选" name="2">
              <div class="check-list">
                <el-checkbox
                  v-model="userStore.filterDoctorAll"
                  @change="onCheckAllDoctor"
                >
                  全选
                </el-checkbox>
                <el-checkbox-group
                  v-model="userStore.filterDoctorList"
                  @change="onCheckListDoctor"
                >
                  <el-checkbox
      ...
      <el-tab-pane label="预约列表" name="2">
        <el-card
          v-for="u in userStore.orderList"
          :key="u.id"
