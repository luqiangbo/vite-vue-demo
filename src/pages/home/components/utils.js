...
export const orderListTrim = (list) => {
  ...
  return list.map((u) => {
    ...
    // 预约详情
    const order = _.find(userStore.orderStatusList, {
      key: order_status || "0",
    });
    console.log({ u, order });
...
function countTimeOverlap(arr) {
  for (let i = 0; i < arr.length; i++) {
    ...
    for (let j = 0; j < arr.length; j++) {
      if (
        i !== j &&
        arr[i].start_time_hm < arr[j].end_time_hm &&
        arr[i].end_time_hm > arr[j].start_time_hm
      ) {
...
export const order2xywhWeek = (orderList) => {
  ...
  orderList.forEach((u) => {
    ...
    const x = week2x(week);
    const y =
      dayDurationHhmm("09:00", dayjs(start_time).format("HH:mm")) / TIME_SIT;
...
function getIndexOverlapCount(dataList) {
  _.forEach(dataList, (data, index) => {
    ...
    for (let i = 0; i < dataList.length; i++) {
      ...
      if (
        data.start_index < otherData.end_index &&
        otherData.start_index < data.end_index
      ) {
        overlapCount += 1;
      }
    }
